...
MCG_SEGMENT_TARGET_DISTANCE_M = 200.0 # Target distance in meters for MCg segments
...
def extract_metrics_from_gpx(gpx_file_path: str, apply_smoothing: bool = True):
    ...
    Includes elevation smoothing, PDD, and MCg calculation.
    """
    distance_km = 0.0
    ...
    try:
        with open(gpx_file_path, 'r', encoding='utf-8') as gpx_file_content:
            ...
            # --- Calculate MCg (Max Climb Gradient over ~30m segments) ---
            # Uses points_for_elevation_metrics (smoothed)
                ...
                for i in range(len(points_for_elevation_metrics) -1):
                    ...
                    for j in range(i + 1, len(points_for_elevation_metrics)):
                        ...
                        if current_segment_dist_m + dist_between_m <= MCG_SEGMENT_TARGET_DISTANCE_M:
                            ...
                            # If adding the next point exceeds target, try to use current segment if it's substantial enough
                            # Or, if the very next segment itself is > target, use that single segment.
                            if dist_between_m > 0 and current_segment_dist_m == 0 : # First segment is already > target
                                current_segment_dist_m = dist_between_m
                                end_point_idx = j
                            break # Segment formed or next point too far

                    # We have a segment from points_for_elevation_metrics[i] to points_for_elevation_metrics[end_point_idx]
                    # with distance current_segment_dist_m
                    if end_point_idx > i and current_segment_dist_m > 1.0: # Ensure segment has length and some distance
            ...
            print(f"MCg (Max Climb Gradient over ~{MCG_SEGMENT_TARGET_DISTANCE_M}m): {mcg:.2f}%")
            ...
            return {
                ...
                "PDD": pdd, 
                "MCg": mcg,
                "ACg": None, # Placeholder
                "ADg": None, # Placeholder
