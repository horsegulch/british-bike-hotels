...
# Constants for ADg (Significant Descent Identification)
SIG_DESCENT_MIN_DISTANCE_M = 500.0     # meters
SIG_DESCENT_MIN_GRADIENT_PERCENT = -3.0 # % (Note: negative for descent)
POTENTIAL_DESCENT_START_GRADIENT_THRESHOLD = -1.0 # % (negative)
...
def extract_metrics_from_gpx(gpx_file_path: str, apply_smoothing: bool = True):
    """
    Parses a GPX file and extracts all six metrics.
    """
    distance_km = 0.0
    ...
    mcg = 0.0 
    acg = 0.0 
    adg = 0.0 # Average Descent Gradient, initialized to 0
        ...
        with open(gpx_file_path, 'r', encoding='utf-8') as gpx_file_content:
            ...
            if len(all_points_original) < 2:
                return {"distance_km": 0.0, "TEGa": 0.0, "TEGa_raw": 0.0, "TEGa_smoothed": 0.0, 
                        "PDD": 0.0, "MCg": 0.0, "ACg": 0.0, "ADg": 0.0}
            ...
            if len(points_for_elevation_metrics) >= 2:
                # print("\n--- Debugging ACg: Identifying Potential Climb Segments ---") # Optional: uncomment for detailed ACg debug
                for i in range(len(points_for_elevation_metrics) - 1):
                    p1, p2 = points_for_elevation_metrics[i], points_for_elevation_metrics[i+1]
                    segment_dist_m = 0.0
                    if p1.latitude is not None and p1.longitude is not None and p2.latitude is not None and p2.longitude is not None:
                    ...
                    if elevation_change_m > 0 and current_segment_gradient >= POTENTIAL_CLIMB_START_GRADIENT_THRESHOLD : 
                        ...
                        if len(current_climb_points) >= 2: 
                            ...
                            if climb_segment_total_dist_m > 0: 
                                ...
                                # if True: print(f"  Potential climb ended. Dist: {climb_segment_total_dist_m:.0f}m, Gain: {climb_segment_total_gain_m:.0f}m, AvgGrad: {avg_grad_of_segment:.2f}%, Factor: {climb_factor:.0f}") # Optional ACg debug
                                if climb_segment_total_dist_m >= SIG_CLIMB_MIN_DISTANCE_M and \
                                   ...
                                   climb_factor >= SIG_CLIMB_FACTOR_THRESHOLD:
                                    ...
                                    # start_idx = points_for_elevation_metrics.index(climb_start_point) # Optional ACg debug
                                    # print(f"    -> QUALIFIED as Significant Climb! Started near point index {start_idx} (approx {cumulative_distances_km_at_points[start_idx]:.2f} km)") # Optional ACg debug
                        current_climb_points = [] 
                if len(current_climb_points) >= 2: # Check last segment
                    ...
                    if climb_segment_total_dist_m > 0:
                        avg_grad_of_segment = (climb_segment_total_gain_m / climb_segment_total_dist_m) * 100.0
                        climb_factor = climb_segment_total_dist_m * avg_grad_of_segment
                        # if True: print(f"  Potential climb ended (end of route). Dist: {climb_segment_total_dist_m:.0f}m, Gain: {climb_segment_total_gain_m:.0f}m, AvgGrad: {avg_grad_of_segment:.2f}%, Factor: {climb_factor:.0f}") # Optional ACg debug
                        if climb_segment_total_dist_m >= SIG_CLIMB_MIN_DISTANCE_M and \
                           avg_grad_of_segment >= SIG_CLIMB_MIN_GRADIENT_PERCENT and \
                           climb_factor >= SIG_CLIMB_FACTOR_THRESHOLD:
                            significant_climb_gradients.append(avg_grad_of_segment)
                            # start_idx = points_for_elevation_metrics.index(climb_start_point) # Optional ACg debug
                            # print(f"    -> QUALIFIED as Significant Climb! Started near point index {start_idx} (approx {cumulative_distances_km_at_points[start_idx]:.2f} km)") # Optional ACg debug
                # if True: print("--- End ACg Debugging ---\n") # Optional ACg debug
            acg = sum(significant_climb_gradients) / len(significant_climb_gradients) if significant_climb_gradients else 0.0

            # --- Calculate ADg (Average Descent Gradient of Significant Descents) ---
            significant_descent_gradients = []
            current_descent_points = []
            if len(points_for_elevation_metrics) >= 2:
                # print("\n--- Debugging ADg: Identifying Potential Descent Segments ---") # Optional: uncomment for detailed ADg debug
                for i in range(len(points_for_elevation_metrics) - 1):
                    p1, p2 = points_for_elevation_metrics[i], points_for_elevation_metrics[i+1]
                    segment_dist_m = 0.0
                    if p1.latitude is not None and p1.longitude is not None and p2.latitude is not None and p2.longitude is not None:
                    ...
                    elevation_change_m = 0.0
                    if p1.elevation is not None and p2.elevation is not None:
                        elevation_change_m = p2.elevation - p1.elevation # Will be negative for descents
                    current_segment_gradient = (elevation_change_m / segment_dist_m * 100.0) if segment_dist_m > 0 else 0

                    if elevation_change_m < 0 and current_segment_gradient <= POTENTIAL_DESCENT_START_GRADIENT_THRESHOLD: # Start or continue a descent
                        if not current_descent_points: current_descent_points.append(p1)
                        current_descent_points.append(p2)
                    else: # Segment is not downhill enough, or is uphill/flat; current potential descent ends
                        if len(current_descent_points) >= 2:
                            ...
                            for k in range(len(current_descent_points) - 1):
                                ...
                                if dp1.elevation is not None and dp2.elevation is not None:
                                    # For descents, elevation_change is negative, so sum abs values for total loss
                                    descent_segment_total_loss_m += abs(min(0, dp2.elevation - dp1.elevation)) 
                            
                            if descent_segment_total_dist_m > 0:
                                # For avg_grad_of_segment, use (total_end_ele - total_start_ele) / total_dist
                                # Or, more simply, (-(total_loss)) / total_dist
                                avg_grad_of_segment = (-descent_segment_total_loss_m / descent_segment_total_dist_m) * 100.0
                                # if True: print(f"  Potential descent ended. Dist: {descent_segment_total_dist_m:.0f}m, Loss: {descent_segment_total_loss_m:.0f}m, AvgGrad: {avg_grad_of_segment:.2f}%") # Optional ADg debug
                                if descent_segment_total_dist_m >= SIG_DESCENT_MIN_DISTANCE_M and \
                                   avg_grad_of_segment <= SIG_DESCENT_MIN_GRADIENT_PERCENT:
                                    significant_descent_gradients.append(abs(avg_grad_of_segment)) # Store absolute gradient
                                    # start_idx = points_for_elevation_metrics.index(descent_start_point) # Optional ADg debug
                                    # print(f"    -> QUALIFIED as Significant Descent! Started near point index {start_idx} (approx {cumulative_distances_km_at_points[start_idx]:.2f} km)") # Optional ADg debug
                        current_descent_points = []
                if len(current_descent_points) >= 2: # Check last segment for descent
                    ...
                    if descent_segment_total_dist_m > 0:
                        avg_grad_of_segment = (-descent_segment_total_loss_m / descent_segment_total_dist_m) * 100.0
                        # if True: print(f"  Potential descent ended (end of route). Dist: {descent_segment_total_dist_m:.0f}m, Loss: {descent_segment_total_loss_m:.0f}m, AvgGrad: {avg_grad_of_segment:.2f}%") # Optional ADg debug
                        if descent_segment_total_dist_m >= SIG_DESCENT_MIN_DISTANCE_M and \
                           avg_grad_of_segment <= SIG_DESCENT_MIN_GRADIENT_PERCENT:
                            significant_descent_gradients.append(abs(avg_grad_of_segment))
                            # start_idx = points_for_elevation_metrics.index(descent_start_point) # Optional ADg debug
                            # print(f"    -> QUALIFIED as Significant Descent! Started near point index {start_idx} (approx {cumulative_distances_km_at_points[start_idx]:.2f} km)") # Optional ADg debug
                # if True: print("--- End ADg Debugging ---\n") # Optional ADg debug
            ...
            print(f"ADg (Average Gradient of Significant Descents): {adg:.2f}%")
            ...
            return {
                ...
                "ADg": adg, "raw_points_count": len(all_points_original)
            }
