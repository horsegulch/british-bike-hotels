...
def serve_index():
    try:
        routes_collection = mongo.db.routes
        users_collection = mongo.db.users # Added for fetching user details
        ...
        def process_route_list_for_index(route_list):
            ...
            for route in route_list:
                # Ensure _id is a string for Jinja links
                if '_id' in route and isinstance(route['_id'], ObjectId):
                    route['_id'] = str(route['_id'])

                # Add creator avatar URL
                route['creator_avatar_url'] = None # Default
                if 'creator_id' in route and route['creator_id']:
                    try:
                        creator_object_id = ObjectId(route['creator_id']) # Ensure it's ObjectId before query
                        creator = users_collection.find_one({"_id": creator_object_id})
                        if creator:
                            if 'avatar_filename' in creator and creator['avatar_filename']:
                                ...
                                route['creator_avatar_url'] = f"https://www.gravatar.com/avatar/{email_hash}?s=40&d=identicon" # s=40 for small avatar
                    except Exception as e_avatar:
                        app.logger.error(f"Error fetching avatar for creator_id {route['creator_id']}: {e_avatar}")

                # Placeholder for static map image - replace with actual logic later
                route['static_map_thumbnail_url'] = "/static/images/map_placeholder.png" # Add a placeholder image to your static/images folder
        ...
        # Fallback if there's an error, to prevent site crash
...
# CORRECTED INDENTATION AND CONTENT FOR list_saved_routes
@app.route('/api/list_saved_routes', methods=['GET'])
def list_saved_routes():
    try:
        routes_collection = mongo.db.routes
        # Using the projection to exclude track_points for efficiency
        all_routes_cursor = routes_collection.find({}, {"track_points": 0})
        
        routes_list = []
        for route in all_routes_cursor:
            # Explicitly convert ObjectId to string for the '_id' field
            if '_id' in route and isinstance(route['_id'], ObjectId):
                route['_id'] = str(route['_id'])
            
            # Also ensure 'processed_at' is a string if it exists and is datetime
            ...
            # Ensure 'creator_id' is a string if it exists and is ObjectId
            if 'creator_id' in route and isinstance(route['creator_id'], ObjectId):
                route['creator_id'] = str(route['creator_id'])

            # If metrics_summary contains datetime or ObjectId, handle them
            ...
            # If comments contain ObjectId or datetime, handle them
            if 'comments' in route and isinstance(route['comments'], list):
                for comment in route['comments']:
                    if isinstance(comment, dict):
                        ...
                        if 'author_id' in comment and isinstance(comment['author_id'], ObjectId):
        ...
        return jsonify(routes_list) # Now jsonify the processed list
...
def profile():
    ...
    # For uploaded_routes
    processed_uploaded_routes = []
    for route in uploaded_routes: # uploaded_routes is the list from mongo.db.routes.find(...)
    if '_id' in route and isinstance(route['_id'], ObjectId):
    route['_id'] = str(route['_id'])
    if 'processed_at' in route and isinstance(route['processed_at'], datetime.datetime):
    route['processed_at'] = route['processed_at'].isoformat()
    if 'creator_id' in route and isinstance(route['creator_id'], ObjectId):
    route['creator_id'] = str(route['creator_id'])
    # Add comprehensive checks for nested fields in metrics_summary and comments if needed,
    # similar to the list_saved_routes function
    if 'metrics_summary' in route and isinstance(route['metrics_summary'], dict):
        for key, value in route['metrics_summary'].items():
    ...
    if 'comments' in route and isinstance(route['comments'], list):
        for comment in route['comments']:
            if isinstance(comment, dict):
                ...
                if 'author_id' in comment and isinstance(comment['author_id'], ObjectId): comment['author_id'] = str(comment['author_id'])
                if 'timestamp' in comment and isinstance(comment['timestamp'], datetime.datetime): comment['timestamp'] = comment['timestamp'].isoformat()
    processed_uploaded_routes.append(route)

    # For favourited_routes
    processed_favourited_routes = []
    for route in favourited_routes: # favourited_routes is the list from mongo.db.routes.find(...)
        if '_id' in route and isinstance(route['_id'], ObjectId):
    route['_id'] = str(route['_id'])
        if 'processed_at' in route and isinstance(route['processed_at'], datetime.datetime):
    route['processed_at'] = route['processed_at'].isoformat()
        if 'creator_id' in route and isinstance(route['creator_id'], ObjectId):
    route['creator_id'] = str(route['creator_id'])
    # Add comprehensive checks for nested fields
    if 'metrics_summary' in route and isinstance(route['metrics_summary'], dict):
         for key, value in route['metrics_summary'].items():
            if isinstance(value, datetime.datetime): route['metrics_summary'][key] = value.isoformat()
            elif isinstance(value, ObjectId): route['metrics_summary'][key] = str(value)
    if 'comments' in route and isinstance(route['comments'], list):
        for comment in route['comments']:
            if isinstance(comment, dict):
                ...
                if 'author_id' in comment and isinstance(comment['author_id'], ObjectId): comment['author_id'] = str(comment['author_id'])
                if 'timestamp' in comment and isinstance(comment['timestamp'], datetime.datetime): comment['timestamp'] = comment['timestamp'].isoformat()
    processed_favourited_routes.append(route)

    return render_template(
    'profile.html',
    user=user_data,
    avatar_url=avatar_url,
    uploaded_routes=processed_uploaded_routes,  # Pass the processed list
    favourited_routes=processed_favourited_routes # Pass the processed list
    )
...
def compare_data_api():
    ...
    # Let Flask's jsonify and the custom JSONEncoder handle everything
