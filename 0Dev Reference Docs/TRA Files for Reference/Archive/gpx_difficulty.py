...
DISTANCE_DIFFICULTY_COEFFICIENT_A = 0.004 # Results in 100km flat = 55 points (5 + 0.005*100^2)

# Uphill Impact Component (Linear UF Model)
MAX_EXPECTED_TEGA = 3500.0
MAX_EXPECTED_ACG = 12.0
MAX_EXPECTED_MCG = 20.0 # Note: This is for normalization, distinct from calculated MCg from GPX
WEIGHT_TEGA = 0.50
WEIGHT_ACG = 0.40
WEIGHT_MCG = 0.10
LINEAR_UF_SLOPE = 0.75  # UF = 1 + LINEAR_UF_SLOPE * US; Max UF = 1.6

# Downhill Reduction Component
PDD_THRESHOLD = 0.65
MAX_ASCENT_FOR_DOWNHILL_REDUCTION = 1000.0
MIN_AVG_DESCENT_GRADIENT_THRESHOLD = 3.0 # Note: This is for eligibility, distinct from calculated ADg
TARGET_ADG_FOR_MAX_REDUCTION = 7.0
WEIGHT_PDD_SCORE = 0.7
WEIGHT_ADG_SCORE = 0.3
MAX_DOWNHILL_REDUCTION_FACTOR = 0.8 # Max 20% reduction
    ...
    try:
        with open(gpx_file_path, 'r', encoding='utf-8') as gpx_file_content:
            ...
            if len(all_points_original) < 2:
                # Return dictionary with default values for all metrics
                return {"distance_km": 0.0, "TEGa": 0.0, "TEGa_raw": 0.0, "TEGa_smoothed": 0.0, 
                        "PDD": 0.0, "MCg": 0.0, "ACg": 0.0, "ADg": 0.0, "raw_points_count": len(all_points_original)}
            ...
            # Calculate TEGa (Smoothed), PDD, and Cumulative Distances
            ...
            final_tega = total_elevation_gain_smoothed # Use smoothed TEGa for algorithm
            ...
            metrics_result = {
                ...
                "TEGa": final_tega, # Smoothed TEGa is used for the algorithm
...
if __name__ == "__main__":
    ...
    # IMPORTANT: Replace with the correct path to YOUR GPX file
    your_actual_gpx_file = r"C:\Users\unwan\Downloads\The Rider.gpx" 
    ...
    if your_actual_gpx_file:
        ...
        if metrics:
            ...
            if all(key in metrics and metrics[key] is not None for key in required_keys):
                ...
                print(f"\n========================================")
                print(f" FINAL CALCULATED DIFFICULTY SCORE: {total_difficulty:.2f}")
                print(f"========================================")
                
            else:
                print("\nCould not calculate total difficulty because some required metrics are missing or None.")
                print("Missing keys or None values for:", 
                      [key for key in required_keys if key not in metrics or metrics[key] is None])

        else:
            print("\nFailed to extract metrics from GPX file. Cannot calculate difficulty.")
    else:
