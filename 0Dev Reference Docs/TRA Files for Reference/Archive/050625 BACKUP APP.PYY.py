...
STATIC_MAP_THUMBNAILS_FOLDER = os.path.join('static', 'map_thumbnails') # Define this
...
def process_route_data(filepath, file_extension, original_identifier, is_upload=False, temp_upload_path=None):
    try:
        ...
        # staticmap expects (longitude, latitude) pairs.
        # Ensure your track_points are in this format. 
        # If get_route_metrics returns (lat, lon), you'll need to swap them:
        # e.g., line_coordinates = [(lon, lat) for lat, lon in track_points]
        # For this example, let's assume track_points from get_route_metrics are already (lon, lat)
        # If not, you MUST adjust the line_coordinates creation.
        line_coordinates = []
        if track_points:
        ...
        if line_coordinates: # Only attempt if there are track points
            try:
                thumbnail_filename = f"{uuid.uuid4().hex}.png"
                thumbnail_save_path = os.path.join(app.config['STATIC_MAP_THUMBNAILS_FOLDER'], thumbnail_filename)
                
                # Create the map object (e.g., 300px wide, 200px high)
                # Adjust width/height as needed for your cards
                m = StaticMap(300, 200) 
                
                # Create a line object from the coordinates
                # Adjust color (e.g., 'blue', '#FF0000') and width as desired
                line = Line(line_coordinates, color="blue", width=3)
                m.add_line(line)
                
                # Render the map to an image and save it
                image = m.render() # You can add zoom=auto here: m.render(zoom_auto=True)
                ...
                # actual_thumbnail_url remains None, so placeholder will be used
        ...
        db_save_data = {
            ...
            "track_points": track_points, # Consider if you need to store all track points if they are very large
            ...
            "actual_thumbnail_url": actual_thumbnail_url # ⭐ ADD THIS NEW FIELD
        }
        ...
        def process_route_list_for_index(route_list):
            ...
            for route in route_list:
                ...
                if 'creator_id' in route and route['creator_id']:
                    try:
                        creator = users_collection.find_one({"_id": ObjectId(str(route['creator_id']))}) # Ensure creator_id is str then ObjectId
                        if creator:
                    ...
                    if route.get('actual_thumbnail_url'): # Check if the key exists and has a value
...
# Corrected route, unindented and with robust ObjectId/datetime to string conversion
@app.route('/api/list_saved_routes', methods=['GET'])
def list_saved_routes():
    try:
        routes_collection = mongo.db.routes
        all_routes_cursor = routes_collection.find({}, {"track_points": 0}) # Exclude track_points
        routes_list = []
...
def get_single_route(route_id):
    try:
        ...
        if route_document: return jsonify(route_document) # app.json_encoder will handle this single doc
...
def unlike_route(route_id):
    try:
        ...
        updated_doc = routes_collection.find_one({"_id": route_obj_id})
        if not updated_doc: return jsonify({"error": "Route not found"}), 404 # Should not happen if result.matched_count was 1
        return jsonify({"likes_count": updated_doc.get("likes_count", 0)}), 200
    except AttributeError: return jsonify({"error": "Database client not available."}), 500
...
def add_comment_to_route(route_id):
    ...
    try:
        ...
        if result.matched_count == 0: return jsonify({"error": "Route not found"}), 404
        # Prepare comment for frontend, ensuring all fields are strings for JSON
...
def route_profile_page(route_id):
    ...
    if current_user.is_authenticated:
        ...
        if user and route_id in [str(fav) for fav in user.get('favorites', [])]: # Compare strings
            is_favorited = True
...
def profile():
    ...
    if 'favorites' in user_data and user_data['favorites']:
        ...
        for fid in user_data['favorites']:
            # Ensure fid is a string before checking validity and converting
            fid_str = str(fid) 
            if ObjectId.is_valid(fid_str): # CORRECTED LINE HERE
    ...
    def process_profile_route_list(route_list_raw):
        ...
        for route in route_list_raw:
            ...
            # Ensure processed_at is an ISO format string
            if 'processed_at' in route and isinstance(route['processed_at'], datetime.datetime):
                route['processed_at'] = route['processed_at'].isoformat()
            
            # Ensure creator_id is a string (if it exists and is an ObjectId)
            if 'creator_id' in route and route['creator_id'] is not None and isinstance(route['creator_id'], ObjectId):
                route['creator_id'] = str(route['creator_id'])
            
            # Process metrics_summary (if it exists and is a dict)
            if 'metrics_summary' in route and isinstance(route['metrics_summary'], dict):
                for key, value in route['metrics_summary'].items():
                    if isinstance(value, datetime.datetime):
                        route['metrics_summary'][key] = value.isoformat()
                    elif isinstance(value, ObjectId): # Should not happen in metrics, but good practice
                        route['metrics_summary'][key] = str(value)
            
            # Process comments (if they exist and is a list)
            if 'comments' in route and isinstance(route['comments'], list):
                for comment in route['comments']:
                    if isinstance(comment, dict):
                        if '_id' in comment and isinstance(comment['_id'], ObjectId):
                            comment['_id'] = str(comment['_id'])
                        if 'author_id' in comment and comment['author_id'] is not None and isinstance(comment['author_id'], ObjectId):
...
def compare_page():
    ...
    obj_ids = [ObjectId(id_str) for id_str in ids_str.split(',') if errors.is_valid(id_str)]
    routes = list(mongo.db.routes.find({"_id": {"$in": obj_ids}}))
    # Ensure routes are processed for Jinja, like in list_saved_routes
    processed_routes = []
    for route in routes:
        if '_id' in route and isinstance(route['_id'], ObjectId): route['_id'] = str(route['_id'])
...
def compare_data_api():
    ...
    for id_str in id_strings:
        if ObjectId.is_valid(id_str): # Check validity before creating ObjectId
            object_ids.append(ObjectId(id_str))
        else:
            app.logger.warning(f"Invalid ObjectId string in compare_data API: {id_str}")

    if not object_ids: # If no valid IDs after filtering
    ...
    if not routes_from_db and id_strings:
        ...
        # Depending on desired behavior, you might return an error or empty list
        # return jsonify({"error": "No matching routes found for comparison"}), 404

    # Let Flask's jsonify and the custom app.json_encoder handle all serialization.
    # This should properly convert ObjectId, datetime, and nested complex types.
